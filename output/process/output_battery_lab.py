...
class Output_Battery_Lab:
    ...
    def output(condition):
        ...
        if condition.saving_path != None and condition.saving_path != '':
            Result_Output.to_csv(data, condition.saving_path, header=header,  length='7') # Save output info into a csv file

        else:
            Result_Output.to_csv(data, condition.config_path, header=header,  length='7') # Save output info into a csv file
